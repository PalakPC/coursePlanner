...
var MongoWebController = {
	...
	configure: function(){	//admin account init check
		config.connection.query('select * from users where username: "admin"', function(err, result){
        //User.findOne({username: "admin"}, function(err, result){
			if (err){
				throw err;
			}
			if (!result){
                connection.query('insert into users values ("admin", "adminadmin")', function(err, rows, fs) {
				new User({
					username: "admin",
					password: "adminadmin",
